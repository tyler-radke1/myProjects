...
func mealTracker(pastMeals: String) -> Meal? {
    if let meal = meals[pastMeals] {
        return meal
    } else {
        return nil
    }
...
mealTracker(pastMeals: "Breakfast")

meals["Lunch"] = Meal(food: ["Chicken Nuggets"], calories: 60)

mealTracker(pastMeals: "Lunch")
/*:
 ...
 */
func mealCheck() -> [String: Any] {
if let meals = UserDefaults.standard.dictionary(forKey: "mealLog") {
    return meals
} else {
    return [:]
}

...
var tyler = User(name: "Tyler", age: 22, height: 65, weight: 120, activity: 2)

print("My name is \(tyler.name), I'm \(tyler.age) years old, I'm \(Int(tyler.height / 12))'\(Int(tyler.height) % 12), I weigh \(tyler.weight) lbs, and on a scale of 1 to 10, I'd rate my activity level as a \(tyler.activity).")
...
struct Distance {
    ...
    init (meters: Double) {
        self.meters = meters
        self.feet = meters * 3.28
    }
    
    init (feet: Double) {
        self.feet = feet
        self.meters = feet / 3.28
    }
}
...
var distance2 = Distance(feet: 234516)
print(distance2)
